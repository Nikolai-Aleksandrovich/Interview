...
初始化模型为一个50维，并迭代训练40次，抛弃出现频率小于2的词
...
注意，这个函数不接受字符串，接受的是字符串列表，这个列表已经被赋予索引
...
另外，因为潜在的训练算法是一个迭代的逼近过程，所以同样的输入会产生不同的输出

### 评估模型

为了评估，首先要获得语料库中每一个文档对应的向量，比较这些向量与训练的语料库的不同，然后依据自相似性返回文档排名，

本质上，输入文档是一种未知的新文档，来检查模型的适应性，以防止过拟合
...
注意到，自身和自身的相似性总是很高，接近1，但是对于第二名的相似性分数低了很多
