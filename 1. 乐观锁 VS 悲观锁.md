...
* AtomicInteger实现了序列化接口，继承了Number类，自然就有序列化ID
* 在类中新建一个Unsafe：获取并操作内存中的数据
* valueOffset：存储value在AtomicInteger中的偏移量
* value：存储在AtomicInteger中的数据值，使用volatile修饰，保证原子性
...
* 首先调用unsafe.getAndAddInt()

* 在unsafe.getAndAddInt()中，首先使用getIntVolatile获得对象o在当前偏移量的值V

* 再使用this.compareAndSwapInt进行对比，判断内存值是否等于V

* 相等，则设置内存值为V+delta

* 不等，则循环并重试

整个“比较+更新”操作封装在compareAndSwapInt()中，在JNI（java本地接口）里是借助于一个CPU指令完成的，属于原子操作，可以保证多个线程都能够看到同一个变量的修改值。
...
- JDK从1.5开始提供了AtomicStampedReference类来解决ABA问题，具体操作封装在compareAndSet()中。compareAndSet()首先检查**当前引用**和**当前标志**与**预期引用**和**预期标志**是否相等，如果都相等，则以原子方式将引用值和标志的值设置为给定的更新值。
...
* 依赖于操作系统Mutex Lock所实现的锁我们称之为“重量级锁”（每次要操作系统对CPU进行状态切换，比较耗费处理器时间），JDK 6中为了减少获得锁和释放锁带来的性能消耗，引入了“偏向锁”和“轻量级锁”。
* 目前锁一共有4种状态，级别从低到高依次是：无锁、偏向锁、轻量级锁和重量级锁。锁状态只能升级不能降级。

![image-20210614162921337](C:/Users/lenovo/AppData/Roaming/Typora/typora-user-images/image-20210614162921337.png)
...
**独享锁**也叫排他锁，是指该锁一次只能被一个线程所持有。如果线程T对数据A加上排它锁后，则**其他线程不能再对A加任何类型的锁**。**获得排它锁的线程即能读数据又能修改数据**。JDK中的synchronized和JUC中Lock的实现类就是互斥锁。
